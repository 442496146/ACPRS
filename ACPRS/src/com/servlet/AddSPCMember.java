...
/**
 * Servlet implementation class AddSPCMember
 */

public class AddSPCMember extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (person1 == null) {
            ...
            request.getRequestDispatcher("manageSPC.jsp").forward(request,response);
        } else if (person2 != null) {
            // 用户已是委员会成员，重新添加
            request.setAttribute("message", message2);
            request.getRequestDispatcher("manageSPC.jsp").forward(request,response);
        } else{
            ChairService.addSPCMember(person1, pc_code);
